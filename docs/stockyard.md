...
A primary use case for Stockyard is to provide a consistent interface for using powerful open models [hosted by various cloud providers](./ai-inference.md). While tools like [Ollama](./ollama.md) are excellent for running models locally, many developers on lightweight workstations need to access larger, faster models in the cloud. Stockyard bridges this gap.
